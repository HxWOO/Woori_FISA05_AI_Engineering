...
| 메서드         | 설명                                |
| -------------- | ----------------------------------- |
| `find(sub)`    | 부분 문자열 첫 인덱스(-1 없으면)     |
| `rfind(sub)`   | 오른쪽부터 첫 인덱스                |
| `count(sub)`   | 부분 문자열 등장 횟수                |

### 변경/치환

| 메서드         | 설명                                |
| -------------- | ----------------------------------- |
| `replace(a,b)` | a를 b로 모두 치환                   |
| `strip()`      | 양쪽 공백 제거                      |
| `lstrip()`     | 왼쪽 공백 제거                      |
| `rstrip()`     | 오른쪽 공백 제거                    |
...
| 메서드         | 설명                                |
| -------------- | ----------------------------------- |
| `split(sep)`   | sep 기준 분리, 리스트 반환           |
| `splitlines()` | 줄바꿈 기준 분리, 리스트 반환        |
| `join(iter)`   | iterable 요소 문자열로 결합          |
...
| ---------------- | --------------------------------- |
| `len(s)`         | 문자열 길이 반환                  |
| `startswith()`   | 접두사로 시작 여부                |
| `endswith()`     | 접미사로 끝나는지                 |
| `isalpha()`      | 알파벳만으로 구성 여부            |
| `isdigit()`      | 숫자만으로 구성 여부              |
| `isalnum()`      | 알파벳/숫자만으로 구성 여부       |
...
| 클래스/함수         | 설명                                   |
| ------------------- | -------------------------------------- |
| `namedtuple()`      | 이름 붙은 필드의 튜플                   |
| `deque`             | 양쪽 끝에서 빠른 추가/삭제              |
| `Counter`           | 해시 가능한 객체 개수 셈                |
| `OrderedDict`       | 삽입 순서 기억 딕셔너리                 |
| `defaultdict`       | 기본값 지정 가능한 딕셔너리             |
| `ChainMap`          | 여러 딕셔너리 묶어 하나의 뷰 제공       |
...
| 함수/메서드         | 설명                                      |
| ------------------- | ----------------------------------------- |
| `re.match()`        | 문자열 **시작**에서 패턴 일치              |
| `re.search()`       | 전체에서 패턴 **처음 등장** 위치 검색       |
| `re.findall()`      | 패턴에 **모두 일치**하는 부분 리스트 반환   |
| `re.finditer()`     | 모두 일치하는 부분을 **이터레이터**로 반환  |
| `re.split()`        | 패턴 기준 분리, 리스트 반환                 |
| `re.sub()`          | 패턴 일치 부분을 **치환**                  |
